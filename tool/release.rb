...
class Release
  ...
  module SubRelease
    ...
    def create_for_github!
      ...
      release =
      gh_client.create_release github_repo, tag, name: tag,
                                                 body: @changelog.release_notes.join("\n").strip,
                                                 prerelease: @version.prerelease?,
                                                 target_commitish: @stable_branch # TODO: target a specific commit

      gh_client.upload_asset release.url, @gem_path, content_type: "application/octet-stream"
      release
    end
    ...
    def latest_release
      @latest_release ||= gh_client.releases(github_repo).select {|release| release.tag_name.start_with?(@tag_prefix) }.max_by do |release|
    ...
    def github_repo
      ENV.fetch("GITHUB_REPO", "rubygems/rubygems")
    end
  end
  ...
  class Bundler
    ...
    def initialize(version, stable_branch)
      ...
      @gem_path = File.expand_path "../bundler/pkg/bundler-#{version}.gem", __dir__
    end
  ...
  class Rubygems
    ...
    def initialize(version, stable_branch)
      ...
      @tag_prefix = "v"
      @gem_path = File.expand_path "../pkg/rubygems-update-#{version}.gem", __dir__
    end
    ...
    def create_for_github!
      release = super
      gh_client.upload_asset release.url, File.expand_path("../pkg/rubygems-#{@version}.zip", __dir__), content_type: "application/zip"
      gh_client.upload_asset release.url, File.expand_path("../pkg/rubygems-#{@version}.tgz", __dir__), content_type: "application/gzip"
      release
    end
  ...
  def initialize(version)
    ...
    @github_repo = ENV.fetch("GITHUB_REPO", "rubygems/rubygems")
  end
  ...
  def scan_unreleased_pull_requests(ids)
    pulls = gh_client.pull_requests(@github_repo, sort: :updated, state: :closed, direction: :desc)
